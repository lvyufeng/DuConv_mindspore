...
then
  ...
  OUTPUT_PATH="../models/mytransformer_kn"
...
candidate_set_file=${INPUT_PATH}/${TASK_NAME}/candidate_set.txt
...
# sample_file=${INPUT_PATH}/resource/${TASK_NAME}/sample.${DATA_TYPE}.txt
...
# candidate_file=${INPUT_PATH}/resource/new_data/candidate.${DATA_TYPE}.txt
# text_file=${INPUT_PATH}/${TASK_NAME}/test.txt
score_file=../output/score.txt
predict_file=../output/predict.txt
# DIR_INPUT_PATH=${INPUT_PATH}/new_data
...
# ${PYTHON_PATH} ../src/construct_candidate.py ${sample_file} ${candidate_set_file} ${candidate_file} 10
...
# inference_model="./models/"+${TASK_NAME}+"/inference_model"

# step 4: predict score by model
$PYTHON_PATH -u ../src/predict.py --task_name ${TASK_NAME} \
                   --use_cuda \
                   --batch_size 10 \
                   --init_checkpoint ${OUTPUT_PATH}/4_8001.ckpt \
                   --data_dir ${DIR_INPUT_PATH} \
...
${PYTHON_PATH} ../src/extract_predict_utterance.py ${candidate_file} ${score_file} ${predict_file}
...
${PYTHON_PATH} ../src/eval.py ${predict_file}
