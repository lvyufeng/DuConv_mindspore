...
class GRU(nn.Cell):
    def __init__(self, input_size, hidden_size, has_bias=True, batch_first=False, dropout=0.0, bidirectional=False):
        ...
        for direction in range(num_directions):
            ...
            self.w_ih_list.append(Parameter(Tensor(np.random.uniform(-stdv, stdv, (input_size, gate_size)).astype(np.float32)),
                                  name='weight_ih{}'.format(suffix)))
            self.w_hh_list.append(Parameter(Tensor(np.random.uniform(-stdv, stdv, (hidden_size, gate_size)).astype(np.float32)),
                                  name='weight_hh{}'.format(suffix)))
            if has_bias:
...
if __name__ == "__main__":
    ...
    net = GRU(10, 16, has_bias=True, batch_first=True, bidirectional=True)
    x = Tensor(np.random.randn(3, 5, 10).astype(np.float16))
    h0 = Tensor(np.random.randn(2, 3, 16).astype(np.float16))
    output, hn = net(x, h0)
    print(output.shape)
    print(hn.shape)
