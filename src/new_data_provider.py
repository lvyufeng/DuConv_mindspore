...
def prepare_batch_data(insts,
    ...
    if 'kn' in task_name: 
        ...
        kn_id, seq_lengths= pad_bath_kn_data(batch_kn_ids, pad_idx=0, max_len=max_len)
    # if 'kn' in task_name:
    #     out = [context_id, context_pos_id, context_segment_id, context_attn_bias, kn_id, labels_list, next_sent_context_index, seq_lengths]
    # else:
    #     out = [context_id, context_pos_id, context_segment_id, context_attn_bias, labels_list, next_sent_context_index, seq_lengths]
    
    # return out
    return context_id, context_pos_id, context_segment_id, context_attn_bias, kn_id, labels_list, next_sent_context_index
    ...
    for item in kn_list:
        if max_len - len(item):
            for i in range(max_len - len(item)):
                item.append(pad_idx)#对kn_ids进行pad
     
    return kn_list,seq_lengths
...
class DataProcessor(object):
    ...
    @classmethod
    def get_labels(self):#MatchProcessor中实现
    ...
    def data_generator(self,
        ...
        def wrapper():
            for batch_data in batch_reader(instance_reader, batch_size):
                context_id, context_pos_id, context_segment_id, context_attn_bias, kn_id, labels_list, next_sent_context_index = self.generate_batch_data(
                    batch_data,
                    voc_size=-1,
                    mask_id=-1,
                    return_attn_bias=True,
                ...
                context_pos_id = np.array(context_pos_id)
                context_segment_id = np.array(context_segment_id)
                context_attn_bias = np.array(context_attn_bias)
                kn_id = np.array(kn_id)
                labels_list = np.array(labels_list)
                next_sent_context_index = np.array(next_sent_context_index)
                yield context_id, context_pos_id, context_segment_id, context_attn_bias, kn_id, labels_list, next_sent_context_index
